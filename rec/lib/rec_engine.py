...
class RecommendationFun(): # pylint: disable=R0903
    ...
    def __init__(self, df_cat, df_user,k : int = 3):
        ...
        self.df_uid = df_user
        self.k = k
        self.dict_catalog_pref= self.df_catalog.set_index(df_cat.columns[0]).T.to_dict('list')
        self.dict_users_pref= self.df_uid.set_index(df_user.columns[0]).T.to_dict('list')
        self.final1=None
    ...
    def rec(self):
        ...
        df_pre_final=pd.DataFrame(self.final1,columns=['User_id','Rest_id','Match','Score'])
